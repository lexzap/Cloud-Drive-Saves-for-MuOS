...
echo "========================================"
echo "  Cloud Download — $OS ($DEVICE_NAME)"
...
echo ""
echo "Downloading save files from ${CLOUD_SAVE_PATH}/..."
...
# Ensure local save dir exists for RockNix if missing
if [ -n "$SAVEFILE_DIR" ] && [ ! -d "$SAVEFILE_DIR" ]; then
    mkdir -p "$SAVEFILE_DIR"
fi

# RockNix has separate savefiles dir — download from cloud savefiles/ subdir
if [ -n "$SAVEFILE_DIR" ]; then
    echo "Downloading in-game save files to ${SAVEFILE_DIR}/..."
    # shellcheck disable=SC2086
    $RCLONE_BINARY copy -P -L --no-check-certificate --update \
        $RCLONE_EXCLUDES \
        "${CLOUD_REMOTE_NAME}:${CLOUD_SAVE_PATH}/savefiles/" "$SAVEFILE_DIR/" \
        --config="$RCLONE_CONFIG" 2>/dev/null || echo "  (savefiles subdir not in cloud yet — skipping)"
fi

# Screenshots (if dir exists)
if [ -d "$SCREENSHOT_DIR" ]; then
    echo "Downloading screenshots to ${SCREENSHOT_DIR}/..."
    $RCLONE_BINARY copy -P -L --no-check-certificate --update \
