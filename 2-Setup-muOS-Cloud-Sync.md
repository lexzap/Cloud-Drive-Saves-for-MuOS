...
3. Set permissions for the rclone binary:

```bash
chmod +x tools/rclone
```

4. Set permissions for task scripts via SSH (these must be run on the device):

```bash
# Set permissions for Goose task scripts (via SSH):
